...
func startCompressWorkers(db *neutronium.DB) compressPool {
...
func (pool compressPool) compress(id int, seq *neutronium.OriginalSeq) int {
...
func compress(db *neutronium.DB, orgSeqId int,
	orgSeq *neutronium.OriginalSeq, mem *memory) neutronium.CompressedSeq {
	...
	cseq := neutronium.NewCompressedSeq(orgSeqId, orgSeq.Name)
	...
	for current = 0; current < olen-mapSeedSize-extSeedSize; current++ {
		...
		for _, seedLoc := range seeds {
			...
			id := neutronium.SeqIdentity(alignment[0], alignment[1])
			...
			// original sequences.
			cseq.Add(neutronium.NewLinkToCoarse(
